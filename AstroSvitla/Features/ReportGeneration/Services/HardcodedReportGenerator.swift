...
actor HardcodedReportGenerator {
    ...
    func generateReport(for area: ReportArea, birthDetails: BirthDetails) async throws -> GeneratedReport {
        ...
        let analysis = detailedAnalysis(for: area, details: birthDetails)
        let recommendations = recommendations(for: area)
    ...
    private func detailedAnalysis(for area: ReportArea, details: BirthDetails) -> String {
        switch area {
        case .finances:
            return """
    ...
    private func recommendations(for area: ReportArea) -> [String] {
        switch area {
        case .finances:
            return [
